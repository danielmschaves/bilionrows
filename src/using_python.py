...
def process_temperatures(csv_path):
    # using positive and negative infinity for comparisons
    ...
    with open(csv_path, 'r', encoding='utf-8') as file:
        ...
        for row in tqdm(csv_reader, total=NUMBER_OF_LINES, desc="Processing"):
    ...
    results = {}
    for station, num_measurements in measurements.items():
        mean_temp = sums[station] / num_measurements
        results[station] = (min_temps[station], mean_temp, max_temps[station])

    print("Statistics calculated. Sorting...")
    # sorting the results by station name
